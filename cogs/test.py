...
class Embed(commands.Cog):
    ...
    async def join(self, interaction: discord.Interaction, member : typing.Union[discord.Member, discord.User]):
        ...
        if isinstance(member, discord.Member):
            ...
            embed = discord.Embed(title=f"{member} just joined {member.guild.name}", description=f"This person joined {humanize.ordinal(pos)} out of {len(member.guild.members)} members",timestamp=member.joined_at,color=random.randint(0, 16777215))
